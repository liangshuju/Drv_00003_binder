...
static void hexdump(void *_data, size_t len)
{
	...
	for (count = 0; count < len; count ++) 
	{
		if ((count & 15) == 0)
			printk("%04zu: ", count);
		printk(" %02x : %c", *data, (*data < 32) || (*data > 126) ? '.' : *data);
		data ++;
			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	/*print data : */
	if (LOG) printk("%s (%d, %d) %s, print data : \n", proc->tsk->comm, proc->pid, thread->pid, __FUNCTION__);
	if (LOG) hexdump(t->buffer->data, tr->data_size);
			...
			void __user *buffer, int size, signed long *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		/* printk info : proc'name, proc id, thread id, cmd'name */
		if (LOG)
